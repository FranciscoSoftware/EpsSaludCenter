...
export default function PersonForm(props:IFormState):JSX.Element {
    ...
    const [user,SetUser] = useState<IUserCrud>({Id:0,UserName:'',UserPassword:'',RoleId:0}); 
    ...
    const getPersonCallBack= (person:any)=>{
        SetUser({Id:person.id,UserName:person.userName,UserPassword:person.userPassword,RoleId:person.roleId})
        SetPerson(
            {
                Id:person.idPerson,
    ...
    const getSpecializationCallBack = (data:ISpecialization[])=>{        
    ...
    async function fetchData() {
        ...
        if(specializations.length==0){
            Get(EnumUrl.GetSpecializations,getSpecializationCallBack)
        }
    }
    ...
    const SpecializationList = ():JSX.Element[]=>{
        ...
        specializations.map((specialization:ISpecialization)=>{
            options.push(<option key={specialization.id} value={specialization.id}>{specialization.specializationName}</option>);
        })        
        return options;
    }
    ...
    const getSpecialization = ():JSX.Element=>{
        return user.RoleId==EnumRoles.Doctor?(
            <Select
                        native
                        value={specializationID}     
                        onChange={(e:React.ChangeEvent<{ name?: string; value: unknown }>)=>{specializationID=Number(e.target.value)}}
            ...
            <form className={classes.form} onSubmit={handleSubmit} >
                ...
                <Grid container direction="row">
                    <Grid item xs={5} sm={5} md={5} >
                        ...
                        name="ImageUrl"              
                        autoFocus
                        value={person.ImageUrl?person.ImageUrl:''}
                        onChange={(e:React.ChangeEvent<HTMLInputElement>)=>SetPerson({...person,ImageUrl:e.target.value})}
